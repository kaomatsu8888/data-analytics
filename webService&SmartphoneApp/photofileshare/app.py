...
app.secret_key = 'dpwvgAxaY2iWHMb2' # セッションIDを暗号化するためのキー

# ログイン処理を実現する --- (*1)
@app.route('/login') # ログイン画面を表示
def login():
    return render_template('login_form.html')

@app.route('/login/try', methods=['POST']) # ログイン処理
def login_try():
    ok = user.try_login(request.form) # ログインを試みる
...
@app.route('/')
@user.login_required # ログインが必要なページ
def index():
