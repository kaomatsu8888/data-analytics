...
def get_template(template_file_path, color=None): # テンプレートファイルのパスを受け取る
    ...
    """
    template = find_template(template_file_path) # テンプレートファイルのパスを取得する
    with open(template, 'r', encoding='utf-8') as template_file: # テンプレートファイルを開く
        contents = template_file.read() # テンプレートファイルの中身を読み込む
        contents = contents.rstrip(os.linesep) # テンプレートファイルの中身の末尾の改行を削除する
        contents = '{splitter}{sep}{contents}{sep}{splitter}{sep}'.format( # テンプレートファイルの中身を整形する
            contents=contents, splitter="=" * 60, sep=os.linesep)
        contents = termcolor.colored(contents, color) # テンプレートファイルの中身をターミナルに表示する
        return string.Template(contents) # テンプレートファイルの中身を返す
